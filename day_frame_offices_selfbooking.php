...
<body>
		...
		if(isset($_GET['d']) && isset($_GET['m']) && isset($_GET['y'])){
			...
			if(mysqli_num_rows($verification_query) == 1){
				...
				switch ($lang){
					...
					case("en"):
						$str = "<div id='selected_day' style='text-align:right;'><b>". $value['d']." ". $value['monthName']." " . $value['y']. "</b> </div>";
						break;
			...
			if(isset($_GET['po'])){
			    $po_e = pack("H*", $_GET['po']);
			    $profile_owner = $crypt->Decrypt($po_e);
			} else {
			    $profile_owner = $userLoggedIn;
			    $po_e = $userLoggedIn_e;
			}
			
			$appointments_calendar = new Appointments_Calendar($con,$profile_owner, $po_e, $selected_year,$selected_month);
			$profile_owner_obj = new User($con, $profile_owner, $po_e);
			...
			$stmt = $con->prepare("SELECT $nick,$adLn1,$adLn2 FROM basic_info_doctors WHERE username=?");
			$stmt->bind_param("s", $userLoggedIn);
			$stmt->execute();
			$off_inf_q = $stmt->get_result();
			$off_inf_arr = mysqli_fetch_array($off_inf_q);
	...
	<div class="day_iframe2" id="style-2">
			<b>
			<?php
				$day_content = $appointments_calendar->getDaySelfBooking($selected_day,$payment_method,$appointment_duration,$appointment_type,$office_arr);
				switch ($lang){
	...
	<?php
		...
		if($reschedule){
			...
			?>
			<script>
				function selectTime4SelfBooking(year,month,day,ap_start,ap_end,profileUsername,payment_method,appointment_type,display_time,display_date){

					$('#modal_display_time', parent.document).html(display_time);
					$('#modal_appo_date', parent.document).html(display_date);
					$('#modal_addln1', parent.document).html('<?php echo $off_inf_arr[$adLn1]; ?>');
					$('#modal_addln3', parent.document).html('<?php echo $off_inf_arr[$adLn2]; ?>');
					$('#modal_nick', parent.document).html('<?php echo $off_inf_arr[$nick]; ?>');

					$( "input[name='year']", parent.document).val(year);
					$( "input[name='month']", parent.document ).val(month);
					$( "input[name='day']", parent.document ).val(day);
					$( "input[name='profile_owner']", parent.document ).val(profileUsername);
					$( "input[name='payment_method']", parent.document ).val(payment_method);
					$( "input[name='ap_id']", parent.document ).val('<?php echo $appo_type_id ?>');
					$( "input[name='ap_st']", parent.document ).val(ap_start);
					$( "input[name='ap_end']", parent.document ).val(ap_end);
					
					$("#confirm_self_booking", parent.document).modal("show");
