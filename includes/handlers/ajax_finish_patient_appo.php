<?php 
	...
	$doctor_obj = new User($con, $did, $crypt->EncryptU($did));
	...
	if($num_aid == 1){
		$stmt = $con->prepare("UPDATE $doc_cal SET confirmed_pat = 1 WHERE consult_id = ?");
		$stmt->bind_param("s", $aid);
		$stmt->execute();
		
		$stmt = $con->prepare("UPDATE $pat_cal SET confirmed_pat = 1 WHERE consult_id = ?");
		$stmt->bind_param("s", $aid);
		$stmt->execute();
